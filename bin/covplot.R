...
### define colors
colors = c(wes_palette(n=4, name="Zissou")[c(2:1,3:4)],"darkblue","orange")[1:2] ## need to change
...
pdf(paste(type,"cov_fig1.pdf",sep="_"))
matplot((small),type="l",lty=1,lwd=3,col=colors,xlab="depth",ylab="",main=type)
legend("topright",colnames(small),fill=colors)
dev.off()
CairoPNG(paste(type,"cov_fig1.png",sep="_"))
matplot((small),type="l",lty=1,lwd=3,col=colors,xlab="depth",ylab="",main=type)
legend("topright",colnames(small),fill=colors)
dev.off()
...
lm_m = lm(ydisp~xcov)
type_scale = (disps-lm_m$coefficients[1])/lm_m$coefficients[2]

pdf(paste(type,"cov_fig2.pdf",sep="_"))
matplot(s_cum,type="l",lty=1,lwd=3,col=colors,xlab="depth",ylab="",main=type)
legend("bottomright",colnames(small),fill=colors)
axis(4,at = xcov,labels = ydisp)
points(y=type_scale,x=sapply(x,function(y) y[[1]]["mu"]),col=colors,pch=19,cex=2)
dev.off()

CairoPNG(paste(type,"cov_fig2.png",sep="_"))
matplot(s_cum,type="l",lty=1,lwd=3,col=colors,xlab="depth",ylab="",main=type)
legend("bottomright",colnames(small),fill=colors)
axis(4,at = xcov,labels = ydisp)
points(y=type_scale,x=sapply(x,function(y) y[[1]]["mu"]),col=colors,pch=19,cex=2)
