...
if [ ! -f ${workpath}/${out}/$NAME.mult_stat.tab ]; then
    printf "#%s\n" "${filetypes[@]}"  > ${workpath}/$out/$NAME.mult_stat.tab

    for f in "${filetypes[@]}"
    do
    ## multi (proper only)
        echo $NAME.$f
        samtools view -f 66  ${workpath}/$out/$NAME.$f |grep "XS:i"|grep -E "@|AS:i" | wc -l >> ${workpath}/$out/$NAME.mult_stat.tab
    done
fi
